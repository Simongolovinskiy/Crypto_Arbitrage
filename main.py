    ...
    def main(self):
        ...
        while True:
            if not self.queue.empty():
                name = self.queue.get()
                threading.Thread(name=name, target=self.run_socket, args=(name,)).start()
                print(f'Перезапускается поток {name}')
                print(f'Количество потоков: {threading.active_count()}')
        ...
        for trading in trade:
            if coin in trading.keys():
                ...
                if len(pair_check) == 3 and self.temp_dict[triple]['Trio_of_price'] != pair_check:
                    ...
                    print(coin_amount, triple)
                    if coin_amount > 1:
                        print(coin_amount, triple)
                        if not self.temp_dict[triple]['Time']:
                            self.temp_dict[triple]['Time'] = datetime.datetime.now()
                    ...
                    if coin_amount > 1:
                        if self.temp_dict[triple]['Time']:
                            self.temp_dict[triple]['Message'] += f"Время: {(datetime.datetime.now() - self.temp_dict[triple]['Time']).total_seconds()}"
